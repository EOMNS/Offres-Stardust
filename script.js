...
async function addQRCodeToPage(doc, qrCodeData, x, y, size) {
  ...
  doc.addImage(qrImageDataURL, 'JPEG', x, y, size, size);
}
...
async function generatePDF() {
    ...
    for (let i = 0; i < sheetData.length; i++) {
      ...
      var dureeOffre = getValeurParNom(sheetData[i], "Durée contrat")
      //var EXP = getValeurParNom(sheetData[i], "Expérience")
    ...
    doc.setFontSize(30);
    const largeurLieu = doc.getTextWidth(lieu);
    const xLieu = (297 - largeurLieu) / 2; // Centrer horizontalement
    doc.text(lieu, xLieu, 100);

    doc.setFont('Arial', 'bold');
    doc.setFontSize(40);
    const largeurNumeroOffre = doc.getTextWidth(`Offre n°${numeroOffre}`);
    const xNumeroOffre = (297 - largeurNumeroOffre) / 2; // Centrer horizontalement
    ...
    const qrCodeData = `https://candidat.pole-emploi.fr/offres/recherche/detail/${numeroOffre}`;
    if (qrCodeData) {
        ...
        await addQRCodeToPage(doc, qrCodeData, qrCodeX, qrCodeY, qrCodeSize);
    }
  ...
  cadrePDF.innerHTML = ''; // Efface le contenu existant
